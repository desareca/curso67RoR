...
select ld.site_id, st.domain_name, date_format(ld.registered_datetime, '%Y %M %d') as date , count(*) as n_leads
from leads ld
...
select concat(cl.first_name, ' ', cl.last_name) as name, 
date_format(bg.charged_datetime, '%M') as month, 
date_format(bg.charged_datetime, '%Y') as year, 
sum(bg.amount) as total_amount 
from billing bg
join clients cl on bg.client_id = cl.client_id
group by cl.client_id, date_format(bg.charged_datetime, '%M %Y')
order by cl.client_id asc, bg.charged_datetime asc;
...
select st.client_id, concat(cl.first_name, ' ', cl.last_name) as clients_name, group_concat(st.domain_name separator ' | ') as sites 
from sites st
join clients cl on cl.client_id = st.client_id
